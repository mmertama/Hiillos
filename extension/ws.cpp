...
const int BUFFER_SIZE{(1024 * 16) - 256};
...
struct Connection {
    ...
    lws_context* context;
    std::function<void (const Extension::Info&)>* connectedCall;
    Connection(const std::string& address, int port, const std::string& protocol, Extension* extension) :
        address{address},
        port{port},
        protocol{protocol},
        interrupted{0},
...
static void sendMessage(Connection* c, const std::string& str) {
...
callback(lws* wsi, lws_callback_reasons reason, void* user, void* in, size_t len)
{
    ...
    case LWS_CALLBACK_PROTOCOL_INIT:
        lws_sul_schedule(context, 0, &mco->sul, connectClient, 1);
        break;

    case LWS_CALLBACK_CLIENT_CONNECTION_ERROR:
        if (lws_retry_sul_schedule(mco->context, 0, &mco->sul, &retry,
                               connectClient, &mco->retries)) {
    ...
    case LWS_CALLBACK_CLIENT_ESTABLISHED:
        ...
        sendMessage(mco, Extension::toString({{"type", "extensionready"}}));
        break;
    ...
    case LWS_CALLBACK_CLIENT_RECEIVE:
        pj =  mco->extension->parse_json(static_cast<const char*>(in));
        if(pj < 0) {
...
static const struct lws_protocols protocols[] = {
        { "hiillos stuff", callback, 0 , BUFFER_SIZE, 0, NULL, BUFFER_SIZE },
        LWS_PROTOCOL_LIST_TERM
};


int startWs(const std::string& address, int port, const std::string& protocol, ExtPtr& ext)
{  
    int logs =
            LLL_USER | LLL_ERR | LLL_WARN | LLL_NOTICE;
    logs |= LLL_INFO | LLL_DEBUG;
    lws_set_log_level(logs, Native::debugPrint);
 ...
 //   memset(mco->msg, 'x', sizeof(mco->msg));
    ...
    ext->m_sendMessage = [&mco](const std::string& msg) {
         if(msg.size() > BUFFER_SIZE)
             return false;
         sendMessage(mco.get(), msg);
...
std::vector<uint8_t> base64Decode(const std::string& data) {
    ...
    std::vector<uint8_t> bytes(1024 * 16);
    const auto sz = lws_b64_decode_string(data.data(), reinterpret_cast<char*>(bytes.data()), static_cast<int>(bytes.size()));
    if(sz <= 0)
        return std::vector<uint8_t>{};
    bytes.resize(sz);
    return bytes;
