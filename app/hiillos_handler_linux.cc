...
void HiillosHandler::PlatformOnCreated(CefRefPtr<CefBrowser> browser) {
    ...
    const Atom nwp = XInternAtom(display, "Hiillos", False);
    XChangeProperty(display, window, nwp, XA_CARDINAL, 32, PropModeReplace, (const unsigned char*) &pid, 1);

 //   const pid_t pid = getpid();
 //   const Atom nwp = XInternAtom(display, "_NET_WM_PID", False);
 //   XChangeProperty(display, window, nwp, XA_CARDINAL, 32, PropModeReplace, (const unsigned char*) &pid, 1);
}
